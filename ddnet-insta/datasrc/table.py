        ...
        if self.default:
            if self.data_type == 'INTEGER':
                if not isinstance(self.default, int):
                    raise ValueError(f"column '{self.name}' has type INTEGER but the default value is not an integer")
            elif self.data_type.startswith('VARCHAR('):
                if not isinstance(self.default, str):
                    raise ValueError(f"column '{self.name}' has type VARCHAR() but the default value is not a string")
