...
from table import AccTable, name_to_snake, split_words, varchar_len
...
class GenExtraTables:
    ...
    def load_tables(self, table_dir: str) -> list[AccTable]:
        ...
        for file in table_path.glob("*.py"):
            ...
            if cls is None:
                ...
                exit(1)
            tab = cls()
            tab.name = split_words(module_name)
            tables.append(tab)
        return tables
    ...
    def data_class(self, table: AccTable) -> str:
        ...
        for col in table.columns:
            if col.data_type == "INTEGER":
                ...
                if col.default:
                    default = col.default
                lines.append(f"        {col.cpp_name()} = {default};")
            elif col.data_type.startswith('VARCHAR('):
                if col.default:
                    lines.append(f"        str_copy({col.cpp_name()}, \"{col.default}\");")
                else:
                    lines.append(f"        {col.cpp_name()}[0] = '\\0';")
            else:
                raise ValueError(f"in table {table.name_camel()} column {col.name_camel()} has unsupported data type '{col.data_type}'")
        lines.append('    }')

        # constructor
        lines.append(f"    CAccountData{table.name_camel()}()")
        lines.append('    {')
        lines.append('        Reset();')
        lines.append('    }')
    ...
    def tables_as_class_members(self) -> str:
        ...
        for tab in self.tables:
            name = tab.name_camel()
            lines.append('        // WARNING: do not use this field!')
            lines.append(f"        std::optional<CAccountData{name}> m_{name} = std::nullopt;")
        lines.append('    } m_LastKnownDbState;')
        return "\n".join(lines)

    def reset_all_data_members(self) -> str:
        lines = []
        if len(self.tables) == 0:
            return "\n"
        for tab in self.tables:
            name = tab.name_camel()
            lines.append(f"        m_{name}.Reset();")
        return "\n" + "\n".join(lines) + "\n"
    ...
    def header(self):
        ...
        {
        public:
            // this uses memory for all tables no matter if they are active in the
            // current mode or not
            // so using a pointer could save some RAM but to avoid memory bugs and keep things
            // simple this is done like this for now
        """)
        code += self.tables_as_class_members()
        code += "\n\n"
        code += "    void Reset()\n"
        code += "    {"
        code += self.reset_all_data_members()
        code += "    }"
        code += textwrap.dedent("""
        };
    ...
    def insert_method(self, table: AccTable) -> str:
        ...
        lines += [
            ...
            '    if(NumInserted != 1)',
            '    {',
            f"""        log_error("sql-thread", "tried to insert one entry into {table_name} but affected %d rows", NumInserted);""",
            '        return false;',
            '    }',
            '',
            '    return true;',
    ...
    def load_method(self, table: AccTable) -> str:
        ...
        lines += [
            ...
            '        {',
            '            log_error("sql-thread", "insert of a new account row into table account_' + table.name_snake().lower() + ' failed");',
            '            return false;',
            '        }',
    ...
    def load_methods(self) -> str:
        ...
        bool CAccountTableCity::Load(class IDbConnection *pSqlServer, int AccountId, CAccount *pAccount, char *pError, int ErrorSize)
        {
            ...
            if(End)
            {
                ...
                if(!Insert(pSqlServer, AccountId, &NewData, pError, ErrorSize))
                {
                    log_error("sql-thread", "insert failed");
                    return false;
                }
