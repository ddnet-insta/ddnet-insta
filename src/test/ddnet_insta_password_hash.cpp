...
TEST(InstaPasswordHash, RandomRange)
{
	for(int i = 0; i < 1000; i++)
	{
		int Value = _random_range(1, 10);
		EXPECT_GE(Value, 1);
		EXPECT_LE(Value, 10);
	}
}

TEST(InstaPasswordHash, RandomDigit)
{
	for(int i = 0; i < 1000; i++)
	{
		char Value = _pass_rand_salt_number();
...
TEST(InstaPasswordHash, GenSalt)
{
	...
	if(!str_contains_only_allowed_chars(STR_ALLOW_ALPHANUMERIC, aSalt))
	{
		log_error("test", "invalid characters in salt '%s'", aSalt);
		EXPECT_EQ(true, false);
	}

	// one byte is taken up by the null term
	// so we only have a string of 15 characters
	EXPECT_EQ(str_length(aSalt), MAX_SALT_LENGTH - 1);
}
...
TEST(InstaPasswordHash, GenHashFullCycle)
{
	char aSalt[MAX_SALT_LENGTH];
	pass_gen_salt(aSalt, sizeof(aSalt));
	char aSaltedHash[512];
	pass_gen_hash_with_salt(aSalt, "my amazing password", aSaltedHash, sizeof(aSaltedHash));
}
