...
void IGameController::Snap(int SnappingClient)
{
	...
	GameInfo.m_RoundStartTick = SnapRoundStartTick(SnappingClient); // ddnet-insta
	GameInfo.m_WarmupTimer = m_Warmup;

	GameInfo.m_ScoreLimit = g_Config.m_SvScorelimit; // ddnet-insta
	GameInfo.m_TimeLimit = SnapTimeLimit(SnappingClient); // ddnet-insta
	...
	SnapGameInfoEx(SnappingClient, &GameInfoEx); // ddnet-insta
	...
	if(Server()->IsSixup(SnappingClient))
	{
		...
		case IGS_GAME_PAUSED:
			...
			break;
		case IGS_END_ROUND:
			GameData.m_GameStateFlags = GameData.m_GameStateFlags & ~protocol7::GAMESTATEFLAG_PAUSED; // clear pause
