...
bool CGameControllerInstaBaseCTF::OnEntity(int Index, int x, int y, int Layer, int Flags, bool Initial, int Number)
{
	...
	for(int RaceTeam = 0; RaceTeam < MAX_CLIENTS; RaceTeam++)
	{
		if(Team == -1 || m_aapFlags[RaceTeam][Team])
			continue;

		CFlag *F = new CFlag(&GameServer()->m_World, Team, RaceTeam);
		F->m_StandPos = Pos;
		F->m_Pos = Pos;
		m_aapFlags[RaceTeam][Team] = F;
...
void CGameControllerInstaBaseCTF::FlagTick()
{
	...
	for(int Team = 0; Team < MAX_CLIENTS; Team++)
	{
		...
		{
			CFlag *pFlag = m_aapFlags[Team][FlagColor];
			...
			if(pFlag->GetCarrier())
			{
				if(m_aapFlags[Team][FlagColor ^ 1] && m_aapFlags[Team][FlagColor ^ 1]->IsAtStand())
				{
					if(distance(pFlag->GetPos(), m_aapFlags[Team][FlagColor ^ 1]->GetPos()) < CFlag::ms_PhysSize + CCharacterCore::PhysicalSize())
					{
						// CAPTURE! \o/
			...
			else
			{
				...
				for(int i = 0; i < Num; i++)
				{
					...
					// only allow flag grabs of the own team
					if(GameServer()->GetDDRaceTeam(apCloseCCharacters[i]->GetPlayer()->GetCid()) != Team)
						continue;
...
void CGameControllerInstaBaseCTF::Snap(int SnappingClient)
{
	...
	for(int Team = 0; Team < MAX_CLIENTS; Team++)
	{
		if(GameServer()->GetDDRaceTeam(SnappingClient) != Team)
			continue;

		int FlagCarrierRed = FLAG_MISSING;
		if(m_aapFlags[Team][TEAM_RED])
		{
			if(m_aapFlags[Team][TEAM_RED]->m_AtStand)
				FlagCarrierRed = FLAG_ATSTAND;
			else if(m_aapFlags[Team][TEAM_RED]->GetCarrier() && m_aapFlags[Team][TEAM_RED]->GetCarrier()->GetPlayer())
				FlagCarrierRed = m_aapFlags[Team][TEAM_RED]->GetCarrier()->GetPlayer()->GetCid();
			else
				FlagCarrierRed = FLAG_TAKEN;
		}

		int FlagCarrierBlue = FLAG_MISSING;
		if(m_aapFlags[Team][TEAM_BLUE])
		{
			if(m_aapFlags[Team][TEAM_BLUE]->m_AtStand)
				FlagCarrierBlue = FLAG_ATSTAND;
			else if(m_aapFlags[Team][TEAM_BLUE]->GetCarrier() && m_aapFlags[Team][TEAM_BLUE]->GetCarrier()->GetPlayer())
				FlagCarrierBlue = m_aapFlags[Team][TEAM_BLUE]->GetCarrier()->GetPlayer()->GetCid();
			else
