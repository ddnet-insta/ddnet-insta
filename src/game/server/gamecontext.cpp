...
void CGameContext::SendRename7(int ClientId)
{
	...
	for(int i = 0; i < Server()->MaxClients(); i++)
	{
		if(i != ClientId)
		{
			...
			m_pController->SendClientDrop7(&Drop, ClientId, i, MSGFLAG_VITAL | MSGFLAG_NORECORD);
			// ddnet-insta uses SendClientInfo7 instead of SendPackMsg
			m_pController->SendClientInfo7(&Info, ClientId, i, MSGFLAG_VITAL | MSGFLAG_NORECORD);
		}
	}
}
...
void CGameContext::OnClientDrop(int ClientId, const char *pReason)
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
		m_pController->SendClientDrop7(&Msg, ClientId, i, MSGFLAG_VITAL | MSGFLAG_NORECORD);
