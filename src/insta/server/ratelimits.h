...
public:
	...
	static void RconCmdLogOrReset(std::vector<CIpRatelimit> &vRatelimits, const NETADDR *pAddr, int CurrentServerTick, const char *pCommand);
	static bool IsLoginRatelimited(const std::vector<CIpRatelimit> &vRatelimits, const NETADDR *pAddr);

	// returns true once if the ratelimit is triggered
	// returns false before and after the ratelimit was reached
	static bool TrackWrongLogin(std::vector<CIpRatelimit> &vRatelimits, const NETADDR *pAddr, int CurrentServerTick);
