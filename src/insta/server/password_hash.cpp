...
{
	dbg_assert(str_length(pSalt) > 1 && str_length(pSalt) <= (int)MAX_SALT_LENGTH, "invalid salt length");
	...
	dbg_assert(str_contains_only_allowed_chars(STR_ALLOW_ALPHANUMERIC, pSalt), "invalid characters in salt");
...
int _random_range(int Min, int Max)
{
	return Min + secure_rand_below(Max - Min + 1);
}

char _pass_rand_salt_letter_lower()
{
	return (char)_random_range(97, 122);
}

char _pass_rand_salt_letter_upper()
{
	return (char)_random_range(65, 90);
}

char _pass_rand_salt_number()
{
	return (char)_random_range(48, 57);
}

char _pass_rand_salt_char()
{
	int Rnd = secure_rand_below(3);
	if(Rnd == 0)
		return _pass_rand_salt_number();
	if(Rnd == 1)
		return _pass_rand_salt_letter_lower();
	if(Rnd == 2)
		return _pass_rand_salt_letter_upper();
	dbg_assert_failed("failed to generate salt char");
}
