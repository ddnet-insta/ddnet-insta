...
bool CAccountsWorker::ChatCmdWorker(IDbConnection *pSqlServer, const ISqlData *pGameData, Write w, char *pError, int ErrorSize)
{
	...
	case EAccountChatCmd::CHAT_CMD_DISPLAY_NAME:
		return ChatCmdDisplayName(pSqlServer, pGameData, pError, ErrorSize);
	case EAccountChatCmd::CHAT_CMD_LOCK_NAME:
		return ChatCmdLockName(pSqlServer, pGameData, pError, ErrorSize);
...
bool CAccountsWorker::ChatCmdDisplayName(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	const auto *pData = dynamic_cast<const CSqlPlayerAccountRequest *>(pGameData);
	auto *pResult = dynamic_cast<CAccountPlayerResult *>(pGameData->m_pResult.get());
...
bool CAccountsWorker::ChatCmdLockName(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	const auto *pData = dynamic_cast<const CSqlPlayerAccountRequest *>(pGameData);
	auto *pResult = dynamic_cast<CAccountPlayerResult *>(pGameData->m_pResult.get());
