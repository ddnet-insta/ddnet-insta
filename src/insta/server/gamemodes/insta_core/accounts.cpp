...
// Not thread-safe!
// this is a copy paste from server.cpp because it can't be accessed here
// not ideal but it works
// would probably be nice if ddnet-insta would provide this in a more global scope
//
// we need a different name to avoid nasty errors
class CInstaRconClientLogger : public ILogger
{
	CServer *m_pServer;
	int m_ClientId;

public:
	CInstaRconClientLogger(CServer *pServer, int ClientId) :
		m_pServer(pServer),
		m_ClientId(ClientId)
	{
	}
...
void CInstaRconClientLogger::Log(const CLogMessage *pMessage)
{
...
void CGameControllerInstaCore::LogoutAccount(class CPlayer *pPlayer, const char *pSuccessMessage, bool IsDisconnect)
{
	...
	// do not bother to do a rename and name owner check on disconnect
	// we can get away without it and reduce complexity
	// but more importantly we do not want to send the rename message
	// in public chat on every disconnect
	if(!IsDisconnect)
	{
...
void CGameControllerInstaCore::LogoutAllAccounts(const char *pSuccessMessage)
{
	for(CPlayer *pPlayer : GameServer()->m_apPlayers)
	{
		...
		LogoutAccount(pPlayer, pSuccessMessage, false);
	}
}
...
void CGameControllerInstaCore::RconForceLogout(int ClientId, const char *pUsername)
{
	...
	if(pVictim)
	{
		...
		LogoutAccount(pVictim, "You got logged out of your account by an admin", false);
		return;
	}
...
void CGameControllerInstaCore::RconLockAccount(int ClientId, const char *pUsername)
{
	...
	if(pVictim)
	{
		...
		LogoutAccount(pVictim, "Your account got locked by an admin", false);
	}
...
void CGameControllerInstaCore::OnAccountDisplayname(int AdminUniqueClientId, const char *pDisplayname, const char *pUsername, bool GotDeleted)
{
	CPlayer *pAdmin = GetPlayerByUniqueId(AdminUniqueClientId);

	// there is another db error log case that will be logged for all users with LOG_ERROR
	char aMessage[1024] = "";
	if(pUsername && pUsername[0])
	{
		if(GotDeleted)
		{
			CPlayer *pOldOwner = GetPlayerByAccountUsername(pUsername);
			if(pOldOwner)
			{
				// TODO: should probably clear out the owner from pOldOwner->m_DisplayName

				char aAdmin[512] = "(server)";
				if(pAdmin)
				{
					str_format(aAdmin, sizeof(aAdmin), "'%s'", Server()->ClientName(pAdmin->GetCid()));
				}

				str_format(
					aMessage,
					sizeof(aMessage),
					"%s removed ownership over the name '%s' from your account",
					aAdmin,
					pDisplayname);
				SendChatTarget(pOldOwner->GetCid(), aMessage);
			}

			str_format(
				aMessage,
				sizeof(aMessage),
				"removed the displayname '%s' ownership from the account '%s'",
				pDisplayname,
				pUsername);
		}
		else
		{
			str_format(
				aMessage,
				sizeof(aMessage),
				"the displayname '%s' is owned by the account '%s'",
				pDisplayname,
				pUsername);
		}
	}
	else
	{
		str_format(
			aMessage,
			sizeof(aMessage),
			"the displayname '%s' has not been claimed yet",
			pDisplayname);
	}
	...
	if(pAdmin)
	{
		...
		CInstaRconClientLogger Logger(pServer, pAdmin->GetCid());
		CLogScope Scope(&Logger);
		log_info("accounts", "%s", aMessage);
	}
	else
	{
		// if the admin running the command left
		// or if it was run from econ/fifo we log it publicly
		log_info("accounts", "%s", aMessage);
	}
}
...
void CGameControllerInstaCore::ProcessAccountRconCmdResult(CAccountRconCmdResult &Result)
{
	...
	case EAccountRconCmd::ACC_DISPLAYNAME:
		OnAccountDisplayname(Result.m_UniqueClientId, Result.m_aDisplayname, Result.m_aUsername, Result.m_GotDeleted);
		break;
	}
}
