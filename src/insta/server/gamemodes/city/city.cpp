...
bool CGameControllerCity::OnFireWeapon(CCharacter &Character, int &Weapon, vec2 &Direction, vec2 &MouseTarget, vec2 &ProjStartPos)
{
	...
	str_format(aBuf, sizeof(aBuf), "+1 chip you now have %d chips", pPlayer->m_Account.m_Mode.m_City.m_Chips++);
	SendChatTarget(pPlayer->GetCid(), aBuf);

	str_format(aBuf, sizeof(aBuf), "+1 food you now have %" PRId64 " foods", pPlayer->m_Account.m_Mode.m_City.m_Food++);
	SendChatTarget(pPlayer->GetCid(), aBuf);
