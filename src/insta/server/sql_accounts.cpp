...
bool CSqlAccounts::CreateAccountsTableThread(IDbConnection *pSqlServer, const ISqlData *pGameData, Write w, char *pError, int ErrorSize)
{
	...
	// TODO: how much sense does the accounts_backup table make?
	//       the create table buffer would be simpler without it
	//       if we create it it should be tested if we can even restore accounts
	//       from a backup table.
	//       How would this even work?
	//       -- register case
	//       The mysql connection to the main database dies
	//       someone tries to register and we save it in sqlite?
	//       then when the connection is back we merge it back to the main db?
	//       What if the same username was already claimed on another server?
	//       -- login case
	//       The mysql connection to the main database dies
	//       someone tries to login and we save it in sqlite?
	//       then when the connection is back we merge it back to the main db?
	//       What if the same account was logged in on another server at the same time?
	//       this is something we want to avoid to disallow double spend
	//       as soon as there is currency
	//       -- conclusion
	//       This makes little sense. This has to be properly revisited and cleaned up
	//       we should not write anything to sqlite for accounts of mysql is on
...
bool CSqlAccounts::ChangePasswordRequest(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	{
		pResult->m_MessageKind = EAccountPlayerRequestType::DIRECT;
		str_copy(pResult->m_aaMessages[0], "Critical database error. Please contact an administrator.");
		return false;
	}
