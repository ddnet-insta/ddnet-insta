...
CONSOLE_COMMAND("acc_ratelimits", "v[victim]?s['reset']", CFGFLAG_SERVER, ConAccountRatelimits, this, "Show ratelimits for given victim, or reset them by passing 'reset'")
CONSOLE_COMMAND("acc_displayname", "s[name]?s['delete']", CFGFLAG_SERVER, ConAccountDisplayname, this, "Check which account claimed the given displayname and also unclaim it when passing 'delete'")
