...
// #define MIN_PASSWORD_LENGTH 3 // use g_Config.m_SvMinPasswordLen
#define MAX_CONTACT_LENGTH 64

// You can pass nullptr for pPassword to only check the username.
// Enforces the current sv_min_password_len config.
bool IsValidUsernameAndPassword(const char *pUsername, const char *pPassword, char *pErrorMsg, size_t ErrorMsgLen);

// You can pass nullptr for pPassword to only check the username.
// Ignores the sv_min_password_len config to allow logging in to old accounts.
